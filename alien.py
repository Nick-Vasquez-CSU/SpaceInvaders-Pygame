...
class AlienFleet:
    alien_exploding_images = [pg.image.load(f'newimg/EXPLODE{n}.png') for n in range(9)]
    alien_images = [[pg.image.load(f'newimg/alien{m}_{n}.png') for n in range (2)] for m in range(4)]
    ...
    def __init__(self, game, v=Vector(1, 0)):
        ...
        self.v = v
        alien = Alien(self.game, alien_index=randint(0,2), image_list=AlienFleet.alien_images)
    ...
    def create_alien(self, row, col):
        ...
        alien = Alien(game=self.game, ul=(x, y), v=self.v, image_list=images, alien_index=randint(0,2))
        self.fleet.add(alien)
...
class Alien(Sprite):
    def __init__(self, game, image_list, alien_index, start_index=0, ul=(0, 100), v=Vector(1, 0),
                 points=1211):
        super().__init__()
        self.game = game
    ...
    def hit(self): 
        ...
        exp = pg.mixer.Sound("explosion.mp3")
        pg.mixer.Sound.play(exp)
    ...
    def draw(self):  
      ...
      if self.dying and self.alien_index == 3:
          X = 600
          Y = 400
          font = pg.font.Font('freesansbold.ttf', 32)
          text = font.render('100', True, YELLOW, WHITE)
